...
// Funds and shares are both STK on the IBKR portal MEXEM introduces onto.
...
// The page is only there to lend its signed-in MEXEM session. Native IBKR
// tabs on interactivebrokers.* are left alone — that walk uses a different
// host and must keep its own cookies. The partnerID in the URL is not the
// introducing broker: a MEXEM dashboard can still say CapTrader1_mig.
let page = null;
...
const outputPath = new URL("mexem-parsed.json", import.meta.url);
const results = [];
const seen = new Set();
...
// A logged-out portal still answers search with `[]`, which the walk would
// treat as "this ISIN is not listed" and finish the catalogue in seconds.
