...
if (startIndex > 1 && fs.existsSync("saxo-parsed.json")) {
  try {
    const existing = JSON.parse(fs.readFileSync("saxo-parsed.json", "utf8"));
...
for (const [queryIndex, query] of queries.entries()) {
  ...
  fs.writeFileSync("saxo-parsed.json", JSON.stringify(results, null, 2));
}
