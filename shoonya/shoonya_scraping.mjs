// Shoonya publishes one public zip per segment, with no login.
//
...
function zipText(buffer) {
  ...
  if (signature !== 0x04034b50) throw new Error("Shoonya archive is not a zip");
  ...
  throw new Error(`Shoonya archive method ${method} is not readable`);
}
...
fs.writeFileSync(new URL("shoonya-parsed.json", import.meta.url), JSON.stringify(stampRows(listings), null, 2));
